...
(declare default-err default-out)
       (enc/defalias         core-appenders/println-appender)
       (enc/defalias         core-appenders/spit-appender)
...
(defn swap-config! [f & args]
         ...
         (apply alter-var-root #'*config* f args))
                 ...
                 (let [rate-limit-specs (:rate-limit appender)
                   ...
                   (when rate-limit-okay?
                     (let [{:keys [async?] apfn :fn} appender
                           ...
                           (delay
                            (output-fn
                                    (assoc data :timestamp_ timestamp_)
                                        ))
